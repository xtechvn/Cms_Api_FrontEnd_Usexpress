...
var _IntervalClientName = null;
$("#ip-kup-search-client").keyup(function (e) {
    if (e.which === 13) {
        _client.OnChangeClientName(e.target.value);
    } else {
        clearInterval(_IntervalClientName);
        _IntervalClientName = setInterval(function () {
            _client.OnChangeClientName(e.target.value);
            clearInterval(_IntervalClientName);
        }, 500);
    }
...
var _client = {
    ...
    OnSearchAdvance: function () {
        let _emailvalue = $('#ip-client-email').val();
        let _phoneValue = $('#ip-client-phone').val();
        let _orderCodeValue = $('#ip-client-order-code').val();

        let objSearch = this.SearchParam;
        objSearch.searchModel.Email = _emailvalue;
        ...
        if (_emailvalue != null && _emailvalue != "") {
            _inputValue += "[Email: " + _emailvalue + "] ";
        }
        ...
        if (_inputValue != "") {
            ...
            $('#ip-kup-search-client').val(_inputValue);
        }
    ...
    OnResetAdvance: function () {
        ...
        this.Search(objSearch);
        $('#ip-client-email').val(_emptyvalue);
        $('#ip-client-phone').val(_emptyvalue);
        $('#ip-client-order-code').val(_emptyvalue);
        $('#ip-kup-search-client').val(_emptyvalue);
